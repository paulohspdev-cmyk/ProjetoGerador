...
if [[ -f "$ENV_FILE" ]]; then
  grep -E '^(RC_ENVIRONMENT|RC_WEB_TLS_MODE|RC_ENABLE_IG200_CONTROL|RC_ENABLE_IG4_LAB_CONTROL|RC_ENABLE_DSE_LAB_CONTROL|RC_AUTH_COOKIE_SECURE|RC_LOGIN_MAX_FAILURES|RC_LOGIN_LOCK_SECONDS|RC_BACKUP_RETENTION|RC_BACKUP_INCLUDE_SECRETS)=' "$ENV_FILE" || true
