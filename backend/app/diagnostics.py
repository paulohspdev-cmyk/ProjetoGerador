...
def _service(name):
    ...
    return {"id": name, "name": name.removesuffix(".service"), "status": "OK" if rc == 0 and out == "active" else "DOWN", "detail": out or "indisponível"}


def _port(host: str, port: int):
    try:
...
def _memory():
    ...
    return {"total": total, "available": avail, "used": max(0, total - avail), "usedPercent": round((total - avail) * 100 / total, 1) if total else 0}
...
def system_diagnostics():
    ...
    return {
        "ok": all(item["status"] == "OK" for item in services if item["id"] in {"scadaserver6.service", "scadacomm6.service"}),
        "services": services,
        "rapid": {
        ...
        "bridge": {
            ...
            "reverseTcp15001Listening": _port("127.0.0.1", 15001),
            "rapidLocal25001Listening": _port("127.0.0.1", 25001),
        },
        "host": {
            "loadAverage": load_avg,
            "memory": _memory(),
            "disk": {"total": usage.total, "used": usage.used, "free": usage.free, "usedPercent": round(usage.used * 100 / usage.total, 1)},
        },
        "generators": [
            {
                "id": g["id"],
