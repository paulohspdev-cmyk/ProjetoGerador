...
func (s *Server) Run(ctx context.Context, sink core.Sink) error {
	...
	for {
		...
		if !netutil.PeerAllowed(remote, allowed) {
			continue
		}
		sink.Publish(core.Event{Kind: core.EventDatagram, ListenerID: s.ID, Transport: "udp", RemoteAddr: remote.String(), LocalAddr: conn.LocalAddr().String(), ProtocolHint: s.ProtocolHint, ReceivedAt: time.Now().UTC(), Payload: append([]byte(nil), buf[:n]...)})
	}
}
