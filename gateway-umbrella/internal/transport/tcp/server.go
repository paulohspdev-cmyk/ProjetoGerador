...
func (s *Server) Run(ctx context.Context, sink core.Sink) error {
	...
	for {
		...
		if !netutil.PeerAllowed(conn.RemoteAddr(), allowed) {
			_ = conn.Close()
			continue
		}
		if s.active.Load() >= int64(s.MaxConnections) {
			_ = conn.Close()
			continue
		}
		s.active.Add(1)
		go func() { defer s.active.Add(-1); s.handle(ctx, conn, sink) }()
	}
}
func (s *Server) handle(ctx context.Context, conn net.Conn, sink core.Sink) {
	...
	base := core.Event{ListenerID: s.ID, SessionID: id, Transport: "tcp", RemoteAddr: conn.RemoteAddr().String(), LocalAddr: conn.LocalAddr().String(), ProtocolHint: s.ProtocolHint}
	ev := base
