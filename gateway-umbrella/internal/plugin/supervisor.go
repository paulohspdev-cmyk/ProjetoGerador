...
func(s *Supervisor)consume(cfg config.Sidecar,r io.Reader)error{sc:=bufio.NewScanner(r);buf:=make([]byte,64*1024);sc.Buffer(buf,4<<20);for sc.Scan(){var wire struct{Kind string `json:"kind"`;SessionID string `json:"sessionId"`;Transport string `json:"transport"`;RemoteAddr string `json:"remoteAddr"`;LocalAddr string `json:"localAddr"`;Protocol string `json:"protocol"`;Payload string `json:"payload"`;Meta map[string]any `json:"meta"`};if err:=json.Unmarshal(sc.Bytes(),&wire);err!=nil{return fmt.Errorf("sidecar %s invalid JSON: %w",cfg.ID,err)};ev:=core.Event{Kind:core.EventSidecar,ListenerID:cfg.ID,SessionID:wire.SessionID,Transport:wire.Transport,RemoteAddr:wire.RemoteAddr,LocalAddr:wire.LocalAddr,ProtocolHint:wire.Protocol,ReceivedAt:time.Now().UTC(),Payload:[]byte(wire.Payload),Meta:wire.Meta};if ev.Transport==""{ev.Transport="sidecar"};if ev.ProtocolHint==""{ev.ProtocolHint=cfg.Protocol};s.Sink.Publish(ev)};return sc.Err()}
