...
func bridgeEndpoint(name string, ep config.Endpoint) bridge.Endpoint {
	return bridge.Endpoint{
		...
		DialTimeout:  time.Duration(ep.DialTimeoutS) * time.Second,
		Reconnect:    time.Duration(ep.ReconnectS) * time.Second,
		KeepAlive:    time.Duration(ep.KeepAliveS) * time.Second,
	}
}
