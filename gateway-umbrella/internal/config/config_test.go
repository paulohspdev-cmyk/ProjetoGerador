...
func TestLoadRejectsCommandPlane(t *testing.T) {
	dir := t.TempDir(); path := filepath.Join(dir, "cfg.json")
	...
	if _, err := Load(path); err == nil { t.Fatal("expected command plane rejection") }
}
func TestLoadRequiresAllowlistOnPublicListener(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(raw), 0o600); err != nil { t.Fatal(err) }
	if _, err := Load(path); err == nil { t.Fatal("expected allowlist rejection") }
}
