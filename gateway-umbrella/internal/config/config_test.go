...
func TestLoadRejectsCommandPlane(t *testing.T) {
	path := writeConfig(t, `{"schema":3,"nodeId":"gw","security":{"commandPlaneEnabled":true},"tunnels":[]}`)
...
func TestLoadRequiresAllowlistOnPublicFieldListener(t *testing.T) {
	path := writeConfig(t, `{
		"schema":3,
		"nodeId":"gw",
		"security":{"requireAllowlist":true},
		"tunnels":[{
			"id":"pusr",
			"field":{"mode":"listen","network":"tcp","bind":"0.0.0.0:15001"},
			"consumer":{"mode":"listen","network":"tcp","bind":"127.0.0.1:25001"}
		}]
	}`)
	if _, err := Load(path); err == nil {
...
func TestLoadAcceptsListenListenTunnel(t *testing.T) {
	path := writeConfig(t, `{
		...
		"tunnels":[{
			"id":"pusr-rapid",
			"field":{"mode":"listen","bind":"0.0.0.0:15003","allowedCidrs":["10.0.0.0/8"]},
	...
	if len(cfg.Tunnels) != 1 || cfg.Tunnels[0].Field.Network != "tcp" {
		t.Fatalf("unexpected tunnel config: %#v", cfg.Tunnels)
	}
}
