...
export type CommandResult = {
  ...
  rpm_before?: number | undefined;
  rpm_after?: number | null | undefined;
};
...
export type WorkOrderApi = {
  ...
  due: number;
  tech: string;
  status: "Urgente" | "Em andamento" | "Planejada" | "Concluída" | "Cancelada" | string;
...
export type SchedulerJob = {
  ...
  enabled: boolean;
  next_run: number;
  last_run?: number | null | undefined;
  last_result: string;
};
export type ControllerPack = {
  packId: string;
  lifecycle: "production" | "lab";
  manufacturer: string;
  ...
  validatedTelemetry?: string[] | undefined;
  notes?: string | undefined;
};
export type ControllerLibrary = {
  packs: ControllerPack[];
  manufacturers: Array<{
    id: string;
    name: string;
    models: number;
  ...
  protocols: Array<{ id: string; name: string; packs: number }>;
  transports: Array<{ id: string; name: string; packs: number }>;
  counts: { total: number; production: number; lab: number };
};
...
export type ReverseTcpListener = {
  generatorId: string;
  tag: string;
  remotePort: number;
  localPort: number;
...
export type BridgeSessionGenerator = {
  generatorId: string;
  tag: string;
  unit: number;
  rapidDeviceNum?: number | null | undefined;
};
...
export type SystemDiagnostics = {
  ...
  host: {
    loadAverage: number[];
    memory: { total: number; available: number; used: number; usedPercent: number } | null;
...
export type OpsBootstrap = {
  ...
  fieldDevices?: FieldDevice[] | undefined;
...
export const rcApi = {
  auth: {
    login: (email: string, password: string, otp?: string) =>
      request<AppUser & { twoFactorEnabled?: boolean }>("/api/auth/login", {
        method: "POST",
    ...
    requestReset: (email: string) =>
      request<{ accepted: boolean }>("/api/auth/password/reset-request", {
        method: "POST",
  ...
  generators: {
    ...
    transportConfig: (id: string) =>
      request<Record<string, unknown>>(
        `/api/generators/${encodeURIComponent(id)}/transport-config`,
      ),
    setTransportConfig: (id: string, config: Record<string, unknown>) =>
      request<Record<string, unknown>>(
        `/api/generators/${encodeURIComponent(id)}/transport-config`,
  ...
  clients: {
    ...
    update: (
      id: string,
      payload: Partial<Pick<OpsClient, "name" | "units" | "gens" | "sla" | "active">>,
    ) =>
      request<OpsClient>(`/api/clients/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(payload),
      }),
    remove: (id: string) =>
      request<void>(`/api/clients/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  sites: {
    ...
    update: (
      ...
      request<OpsSite>(`/api/sites/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(payload),
      }),
    remove: (id: string) =>
      request<void>(`/api/sites/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  workOrders: {
    ...
    update: (
      id: string,
      payload: Partial<Pick<WorkOrderApi, "type" | "due" | "tech" | "status" | "description">>,
    ) =>
    ...
    remove: (id: string) =>
      request<void>(`/api/work-orders/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  agenda: {
    ...
    update: (
      id: string,
      payload: Partial<
        Pick<AgendaItemApi, "title" | "when" | "site" | "generatorId" | "kind" | "enabled">
      >,
    ) =>
      request<AgendaItemApi>(`/api/agenda/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(payload),
      }),
    remove: (id: string) =>
      request<void>(`/api/agenda/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  rules: {
    list: () => request<AutomationRuleApi[]>("/api/automation/rules"),
    create: (payload: { name: string; trigger: string; action: string }) =>
      request<AutomationRuleApi>("/api/automation/rules", {
        method: "POST",
        body: JSON.stringify(payload),
      }),
    update: (
      id: string,
      payload: Partial<Pick<AutomationRuleApi, "name" | "trigger" | "action" | "enabled">>,
    ) =>
    ...
    approve: (id: string) =>
      request<AutomationRuleApi>(`/api/automation/rules/${encodeURIComponent(id)}/approve`, {
        method: "POST",
      }),
    enable: (id: string, enabled: boolean) =>
      request<AutomationRuleApi>(`/api/automation/rules/${encodeURIComponent(id)}/enabled`, {
        method: "PUT",
        body: JSON.stringify({ enabled }),
      }),
    remove: (id: string) =>
      request<void>(`/api/automation/rules/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  reports: {
    ...
    remove: (id: string) =>
      request<void>(`/api/reports/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  webhooks: {
  ...
  backups: {
    ...
    download: (id: string) =>
      download(`/api/backups/${encodeURIComponent(id)}/download`, `${id}.tar.gz`),
    remove: (id: string) =>
      request<void>(`/api/backups/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  alarms: {
    acknowledgements: () => request<AlarmAckApi[]>("/api/alarms/ack"),
    acknowledge: (alarmKey: string) =>
      request<AlarmAckApi>("/api/alarms/ack", {
        method: "POST",
  ...
  fieldDevices: {
    list: (kind?: "modem" | "gateway") =>
      request<FieldDevice[]>(`/api/field-devices${kind ? `?kind=${kind}` : ""}`),
    create: (payload: Omit<FieldDevice, "id" | "active"> & { active?: boolean | undefined }) =>
      request<FieldDevice>("/api/field-devices", { method: "POST", body: JSON.stringify(payload) }),
    update: (id: string, payload: Partial<FieldDevice>) =>
      request<FieldDevice>(`/api/field-devices/${encodeURIComponent(id)}`, {
        method: "PATCH",
        body: JSON.stringify(payload),
      }),
    remove: (id: string) =>
  ...
  notifications: {
    ...
    test: (channel: "panel" | "email" | "whatsapp" | "webhook", destination = "") =>
      request<{ id: number; status: string }>("/api/notifications/test", {
        method: "POST",
  ...
  scheduler: {
    ...
    create: (payload: {
      id?: string | undefined;
      name: string;
      kind: "backup" | "report" | "notification";
      interval_seconds: number;
      payload?: Record<string, unknown> | undefined;
      enabled?: boolean | undefined;
    }) =>
      request<SchedulerJob>("/api/scheduler", { method: "POST", body: JSON.stringify(payload) }),
    update: (
      id: string,
      payload: {
        name: string;
      ...
      request<SchedulerJob>("/api/scheduler", {
        method: "POST",
        body: JSON.stringify({ ...payload, id }),
      }),
    remove: (id: string) =>
      request<void>(`/api/scheduler/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  apiTokens: {
    ...
    revoke: (id: string) =>
      request<void>(`/api/api-tokens/${encodeURIComponent(id)}`, { method: "DELETE" }),
  },
  system: {
