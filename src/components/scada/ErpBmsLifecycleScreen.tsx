...
export function ErpBmsLifecycleScreen() {
  ...
  const toggle = async (row: WebhookApi) => {
    try {
      ...
      setMessage(row.status === "Ativo" ? "Conector pausado." : "Conector ativado.");
      setError("");
      await load();
    } catch (err) {
      setError(errorText(err));
  ...
  return (
    <ScreenBody>
        ...
        items={[
          {
            icon: Landmark,
            label: "ERP/BMS HTTP",
            value: status?.configured ? "ATIVO" : "NÃO CONFIGURADO",
      ...
      <p className="rounded-md border border-border bg-card px-3 py-2 text-[11px] text-muted-foreground">
        ERP/BMS usa HTTP/webhook exclusivamente para saída de dados e eventos. Para leitura
        estruturada, use a API com token. Webhook recebido nunca executa START, STOP, MCB, GCB,
        transferência ou paralelismo.
      </p>
      {status?.adapter && (
        <p className="rounded-md border border-border bg-card px-3 py-2 text-[11px] text-muted-foreground">
          Adaptador: <b>{status.adapter}</b>
      ...
      {admin && (
        <Panel title={editing ? "Editar conector HTTP" : "Novo conector HTTP"}>
          <form onSubmit={save} className="grid gap-2 md:grid-cols-[1fr_220px_auto]">
            <input
              ...
              placeholder="https://erp.exemplo.com/rc/events"
      ...
      <Panel title="Conectores persistidos">
        <ScadaTable
          ...
          columns={[
            ...
            {
              label: "Destino",
              render: (r) => <span className="num break-all text-[11px]">{r.url}</span>,
            },
            {
              label: "Estado",
