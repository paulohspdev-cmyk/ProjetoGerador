...
export function SchedulesV3Screen() {
  ...
  const reset = () => {
    setEditing(null); setName(""); setKind("backup"); setInterval("86400"); setReportFormat("PDF"); setChannel("panel"); setDestination("");
  };

  const beginEdit = (row: SchedulerJob) => {
    const jobKind = row.kind as JobKind;
    setEditing(row); setName(row.name); setKind(jobKind); setInterval(String(row.interval_seconds));
  ...
  const buildPayload = () => kind === "report"
  ...
  const save = async (e: FormEvent) => {
    ...
    if (!Number.isInteger(intervalSeconds) || intervalSeconds < 60) { setError("O intervalo mínimo é 60 segundos."); return; }
    setBusy(true);
    try {
      const payload = buildPayload();
      if (editing) {
        await rcApi.scheduler.update(editing.id, { name: name.trim(), kind, interval_seconds: intervalSeconds, payload, enabled: editing.enabled });
        setMessage("Agendamento atualizado.");
      } else {
        await rcApi.scheduler.create({ name: name.trim(), kind, interval_seconds: intervalSeconds, payload, enabled: true });
        setMessage("Agendamento criado.");
      }
      reset(); await load();
    } catch (err) { setError(err instanceof Error ? err.message : "Falha ao salvar agendamento."); }
  ...
  const toggle = async (row: SchedulerJob) => {
    try {
      ...
      setMessage(row.enabled ? "Agendamento pausado." : "Agendamento reativado."); setError(""); await load();
    } catch (err) { setError(err instanceof Error ? err.message : "Falha ao alterar agendamento."); }
  };

  const remove = async (row: SchedulerJob) => {
    if (!window.confirm(`Excluir o job administrativo ${row.name}?`)) return;
    try { await rcApi.scheduler.remove(row.id); if (editing?.id === row.id) reset(); setMessage("Agendamento excluído."); await load(); }
  ...
  return <ScreenBody>
    ...
    <p className="rounded-md border border-alert/30 bg-alert/5 px-3 py-2 text-[11px] text-muted-foreground">Scheduler fechado: aceita exclusivamente backup, relatório e notificação. START, STOP, transferência, MCB, GCB e paralelismo não fazem parte deste motor.</p>
    ...
    {admin && <Panel title={editing ? "Editar job administrativo" : "Novo job administrativo"}>
      <form onSubmit={save} className="grid gap-2 md:grid-cols-4">
        ...
        <div className="flex items-end gap-1"><button disabled={busy} className="h-9 rounded-md bg-primary px-4 text-sm font-bold text-primary-foreground disabled:opacity-50">{busy ? "Salvando…" : editing ? "Salvar" : "Criar job"}</button>{editing && <button type="button" onClick={reset} className="h-9 rounded-md border border-border px-3 text-xs">Cancelar</button>}</div>
      </form>
    </Panel>}

    <Panel title="Scheduler administrativo">
      <ScadaTable rows={rows} min="940px" columns={[
        ...
        { label: "Ações", render: (r) => admin ? <span className="flex flex-wrap gap-1"><ActionBtn onClick={() => beginEdit(r)}>Editar</ActionBtn><ActionBtn onClick={() => void toggle(r)}>{r.enabled ? "Pausar" : "Reativar"}</ActionBtn><ActionBtn tone="danger" onClick={() => void remove(r)}>Excluir</ActionBtn></span> : "—" },
      ]} />
    </Panel>
  </ScreenBody>;
}
