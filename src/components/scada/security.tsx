...
function NoAccess() {
  return <ScreenBody><div className="rounded-md border border-border bg-card px-4 py-3 text-sm text-muted-foreground">Seu perfil não possui permissão para este módulo.</div></ScreenBody>;
}

export function RolesScreen() {
  return <ScreenBody>
    <Stats items={[{ icon: ShieldCheck, label: "Perfis", value: ROLE_META.length }]} />
    <Panel title="Perfis e permissões"><ScadaTable rows={ROLE_META} columns={[
...
export function AuditScreen() {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    void rcApi.audit.list(300).then((items) => { if (mounted) setRows(items); }).catch((err) => { if (mounted) setError(err instanceof Error ? err.message : "Falha ao carregar auditoria."); });
  ...
  return <ScreenBody>
    ...
    <Panel title="Auditoria real do sistema"><ScadaTable rows={rows} columns={[
