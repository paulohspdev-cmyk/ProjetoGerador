  ...
  return (
    <ScreenBody>
      ...
      <p className="rounded-md border border-border bg-card px-3 py-2 text-[11px] text-muted-foreground">
        Este inventário contém somente dados administrativos informados. Estado físico da sessão TCP aparece em
        Equipamentos → Conectividade; IMEI, SIM, RSSI e conexão nunca são inferidos.
      </p>
      ...
      {admin && (
        <Panel title={editing ? `Editar ${kind}` : `Cadastrar ${kind}`}>
          <form onSubmit={save} className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
            ...
            {kind === "modem" && (
              <>
                <input value={imei} onChange={(e) => setImei(e.target.value)} placeholder="IMEI" className="h-9 rounded-md border border-input bg-background px-2 text-sm" />
                <input value={sim} onChange={(e) => setSim(e.target.value)} placeholder="SIM / ICCID" className="h-9 rounded-md border border-input bg-background px-2 text-sm" />
                <input value={carrier} onChange={(e) => setCarrier(e.target.value)} placeholder="Operadora" className="h-9 rounded-md border border-input bg-background px-2 text-sm" />
              </>
            )}
            <div className="flex gap-1">
              <button disabled={busy} className="h-9 rounded-md bg-primary px-4 text-sm font-bold text-primary-foreground disabled:opacity-50">
              ...
              { label: "Cadastro", render: (r) => <Pill tone={r.active ? "ok" : "muted"}>{r.active ? "Ativo" : "Inativo"}</Pill> },
              {
                ...
                render: (r) => admin ? (
                  <span className="flex flex-wrap gap-1">
                    ...
                    <ActionBtn onClick={() => void toggle(r)}>{r.active ? "Desativar" : "Ativar"}</ActionBtn>
                    <ActionBtn tone="danger" onClick={() => void remove(r)}>Excluir</ActionBtn>
                  </span>
                ) : "—",
              },
            ]}
...
export function ConnectivityScreen() {
  ...
  return (
    <ScreenBody>
      ...
      <p className="rounded-md border border-border bg-card px-3 py-2 text-[11px] text-muted-foreground">
        A tabela física abaixo vem do processo da bridge. Se o arquivo de status estiver ausente ou vencido, o painel mostra N/D — nunca converte ausência de evidência em “offline”. Contadores são desde o início do processo atual da bridge.
      </p>
      <Panel title="Sessões físicas modem / DTU → gateway → bridge">
        {!sessions.length ? (
          <p className="py-8 text-center text-sm text-muted-foreground">
            {available ? "Nenhuma porta reverse TCP publicada pela bridge." : "Status físico da bridge ainda não disponível nesta máquina."}
          </p>
        ) : (
          <ScadaTable
