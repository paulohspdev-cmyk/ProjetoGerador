  ...
  return (
    <ScreenBody>
      ...
      <Panel title="Clientes">
        <ScadaTable rows={clients} columns={[
...
export function UnitsScreen() {
  ...
  const load = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Falha ao consultar unidades.");
    }
  };
  ...
  const onCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      setName(""); setCity(""); setState(""); setAddress(""); setLatitude(""); setLongitude("");
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao cadastrar unidade.");
    }
  };
...
export function AuditScreen() {
  ...
  return <ScreenBody><Stats items={[{ icon: ScrollText, label: "Eventos de auditoria", value: rows.length }]} /><ErrorBox message={error} /><Panel title="Auditoria real"><ScadaTable rows={tableRows} columns={[
...
export function ApiScreen() {
  ...
  return <ScreenBody><Stats items={[{ icon: Plug, label: "API", value: health?.ok ? "ONLINE" : "—", tone: health?.ok ? "text-online" : "text-muted-foreground" }, { icon: Plug, label: "Versão", value: health?.version ?? "—" }]} /><ErrorBox message={error} /><Panel title="API interna RC Geradores"><div className="space-y-2 text-[13px]">
    ...
    <p className="rounded-md border border-border p-3">Tokens externos/JWT e rate limit ainda não estão habilitados; a tela não simula chaves inexistentes.</p>
