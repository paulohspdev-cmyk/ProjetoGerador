...
export function AgendaV3Screen() {
  ...
  const beginEdit = (row: AgendaItemApi) => {
    setEditing(row.id);
    setTitle(row.title);
    setWhen(row.when);
    setSite(row.site || "");
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (editing)
        await rcApi.agenda.update(editing, {
          title: title.trim(),
          when: when.trim(),
          site: site.trim(),
        });
      ...
      setError(err instanceof Error ? err.message : "Falha ao salvar compromisso.");
    } finally {
      setBusy(false);
    }
  };
  const toggle = async (row: AgendaItemApi) => {
    try {
      await rcApi.agenda.update(row.id, { enabled: row.enabled === false });
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao alterar compromisso.");
    }
  };
  const remove = async (row: AgendaItemApi) => {
    if (!window.confirm(`Excluir o compromisso “${row.title}”?`)) return;
    try {
      await rcApi.agenda.remove(row.id);
      if (editing === row.id) reset();
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao excluir compromisso.");
    }
  };
      ...
      <Stats
        items={[
          { icon: CalendarDays, label: "Compromissos", value: rows.length },
          {
            icon: CalendarDays,
            label: "Ativos",
            value: rows.filter((row) => row.enabled !== false).length,
            tone: "text-online",
          },
          {
            icon: CalendarDays,
        ...
        <Panel title={editing ? "Editar compromisso" : "Novo compromisso"}>
          <form onSubmit={submit} className="grid gap-2 sm:grid-cols-4">
            ...
            <label className="text-[11px] font-semibold text-muted-foreground">
              Local
              <input
                list="agenda-v3-sites"
                value={site}
                onChange={(e) => setSite(e.target.value)}
                className="mt-1 h-9 w-full rounded-md border border-input bg-background px-2 text-sm"
              />
              <datalist id="agenda-v3-sites">
                {sites.map((item) => (
            ...
            { label: "Tipo", render: (r) => r.kind || "manual" },
            {
              ...
              render: (r) => (
                <Pill tone={r.enabled === false ? "muted" : "ok"}>
                  {r.enabled === false ? "Cancelado" : "Ativo"}
                </Pill>
              ),
            },
            {
