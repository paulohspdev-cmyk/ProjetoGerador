...
function useRemote<T>(loader: () => Promise<T>, initial: T) {
  ...
  useEffect(() => {
    ...
    loader()
      ...
      .catch((err) => {
        if (active) setError(err instanceof Error ? err.message : "Falha ao consultar API");
      })
      .finally(() => {
...
export function ChannelsScreen() {
  const {
    ...
    error,
    loading,
  } = useRemote<ChannelCatalogItem[]>(() => rcApi.library.channels(), []);
  return (
    <ScreenBody>
        ...
        {!!rows.length && (
          <ScadaTable
            ...
            columns={[
              ...
              {
                label: "Acesso",
                render: (r) => <Pill tone={r.access === "R" ? "ok" : "warn"}>{r.access}</Pill>,
              },
...
export function TemplatesScreen() {
  const { data, error, loading } = useRemote<ControllerLibrary | null>(
    () => rcApi.library.get(),
    null,
  );
    ...
    <ScadaTable
      ...
      columns={[
        { label: "Componente", render: (r) => <b>{r.name}</b> },
        {
          label: "Status",
          render: (r) => (
            <Tone tone={r.status === "active" || r.status === "OK" ? "ok" : "warn"}>
              {r.status}
            </Tone>
          ),
        },
          ...
          <>
            ...
            <div className="mt-3 grid gap-2 text-[12px] sm:grid-cols-3">
              <div className="rounded-md border border-border p-2">
                Memória: <b>{data.host.memory ? `${data.host.memory.usedPercent}%` : "N/D"}</b>
              </div>
              <div className="rounded-md border border-border p-2">
                Disco: <b>{data.host.disk ? `${data.host.disk.usedPercent}%` : "N/D"}</b>
              </div>
              <div className="rounded-md border border-border p-2">
                Geradores: <b>{data.generators.length}</b>
              </div>
            </div>
          </>
        )}
    ...
    <ScreenBody>
      <Stats
        items={[
          ...
          {
            icon: Library,
            label: "Modelos no catálogo",
            value: lib?.counts.catalogTotal ?? data?.counts.total ?? 0,
          },
          {
            icon: Package,
            label: "Provisionáveis",
            value: lib?.counts.catalogProvisionable ?? data?.counts.production ?? 0,
            tone: "text-online",
          },
        ]}
      />
        ...
        {!!rows.length && (
          <ScadaTable
            ...
            columns={[
              ...
              {
                label: "Produção",
                render: (r) => <Tone tone={r.production ? "ok" : "muted"}>{r.production}</Tone>,
              },
      ...
      <Stats
        items={[
          { icon: Library, label: "Modelos catalogados", value: catalog.length },
          {
            icon: Package,
            label: "Produção",
            value: catalog.filter((item) => item.provisionable).length,
            tone: "text-online",
          },
          {
            icon: FlaskConical,
            label: "Em lab",
            value: catalog.filter((item) => item.packLifecycle === "lab").length,
          },
          {
            icon: Info,
            label: "A homologar",
            value: catalog.filter((item) => !item.packLifecycle).length,
          },
        ]}
      />
      ...
      <Stats
        items={[
          { icon: Package, label: "Perfis homologados", value: data?.counts.total ?? 0 },
          {
            icon: Package,
            label: "Produção",
            value: data?.counts.production ?? 0,
            tone: "text-online",
          },
          ...
          {data?.packs.map((p) => (
            ...
            >
              <span>
                ...
                · schema {String((p as { schema?: number }).schema ?? "—")} · {p.status}
              </span>
    ...
    <ScreenBody>
      <Stats
        items={[
          ...
          {
            icon: Library,
            label: "Fila de homologação",
            value: lib?.counts.catalogInventoryOnly ?? 0,
          },
        ]}
      />
        ...
        <div className="space-y-2">
          {lab.map((p) => (
            <div key={p.packId} className="rounded-md border border-border p-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                {p.notes || "Ainda não homologado para produção."}
              </p>
            </div>
          ))}
        </div>
...
export function VersionScreen() {
  const { data, error, loading } = useRemote<SystemDiagnostics["version"] | null>(
    () => rcApi.system.version(),
    null,
  );
  return (
    <ScreenBody>
      ...
      <Panel title="Configurações da interface">
        ...
        >
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground">
              Preferência visual local; não altera configuração industrial.
            </p>
          </div>
...
export function BackupsScreen() {
  ...
  return (
    <ScreenBody>
      ...
      >
        {!backups.length && (
          <p className="text-[12px] text-muted-foreground">Nenhum backup executado.</p>
        )}
        {!!backups.length && (
          <ScadaTable
            rows={backups}
            columns={[
              ...
              {
                label: "Resultado",
                render: (r) => <Tone tone={r.result === "OK" ? "ok" : "warn"}>{r.result}</Tone>,
              },
            ]}
          />
        )}
