...
export function SystemSettingsV3Screen() {
  ...
  const loadSecurity = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Falha ao consultar segurança da conta.");
    }
  };
  useEffect(() => {
    void loadSecurity();
  ...
  const changePassword = async (event: FormEvent) => {
    ...
    try {
      ...
      setConfirmPassword("");
      setMessage(
        "Senha alterada. As demais sessões foram revogadas pelo backend quando aplicável.",
      );
      await loadSecurity();
    } catch (err) {
  ...
  const setup2fa = async () => {
    ...
    setError("");
    try {
      const result = await rcApi.auth.setup2fa();
      setSecret(result.secret);
      setUri(result.otpauthUri);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao iniciar 2FA.");
    }
  };
  const enable2fa = async () => {
    ...
    setError("");
    try {
      await rcApi.auth.enable2fa(otp.trim());
      setOtp("");
      setSecret("");
      setUri("");
  ...
  const revokeSessions = async () => {
    if (
      !window.confirm(
        "Revogar todas as sessões desta conta? Você poderá precisar entrar novamente.",
      )
    )
      return;
    try {
      await rcApi.auth.revokeSessions();
    ...
    <ScreenBody>
      <Stats
        items={[
          { icon: Settings, label: "Tema", value: theme === "dark" ? "ESCURO" : "CLARO" },
          {
            icon: ShieldCheck,
            label: "2FA",
            value: twoFactor ? "ATIVO" : "INATIVO",
            tone: twoFactor ? "text-online" : undefined,
          },
        ...
        >
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Preferência local; não altera configuração industrial.
            </p>
          </div>
          <Pill tone={theme === "dark" ? "ok" : "muted"}>{theme === "dark" ? "ON" : "OFF"}</Pill>
        </button>
      </Panel>
      ...
      <Panel title="Alterar senha">
        <form onSubmit={changePassword} className="grid gap-2 md:grid-cols-4">
          ...
          <div className="flex items-end">
            <button
              disabled={busy}
              className="h-9 w-full rounded-md bg-primary px-3 text-sm font-bold text-primary-foreground disabled:opacity-50"
            >
      ...
      <Panel title="Autenticação em dois fatores">
        <div className="space-y-3 text-[12px]">
          {!twoFactor && !secret && (
            <ActionBtn onClick={() => void setup2fa()}>
              <KeyRound className="mr-1 inline size-3" />
              Configurar 2FA
            </ActionBtn>
          )}
          {!twoFactor && secret && (
            <div className="space-y-2 rounded-md border border-alert/30 bg-alert/5 p-3">
              <p>
                <b>1.</b> Adicione a conta no autenticador usando a chave abaixo ou a URI otpauth.
              </p>
              <code className="block break-all rounded bg-background p-2">{secret}</code>
              <code className="block break-all rounded bg-background p-2 text-[10px]">{uri}</code>
              <p>
                <b>2.</b> Digite o código atual para confirmar.
              </p>
              <div className="flex gap-2">
                ...
                />
                <ActionBtn disabled={otp.length < 6} onClick={() => void enable2fa()}>
                  Habilitar 2FA
                </ActionBtn>
                <ActionBtn
                  onClick={() => {
      ...
      <Panel
        ...
        actions={
          sessions.length ? (
            <ActionBtn tone="danger" onClick={() => void revokeSessions()}>
              Revogar todas
            </ActionBtn>
          ) : undefined
        }
          ...
          columns={[
            {
              label: "Criada",
              render: (r) => (
                <span className="num">{new Date(r.createdAt * 1000).toLocaleString("pt-BR")}</span>
              ),
            },
            {
              label: "Última atividade",
              render: (r) => (
                <span className="num">{new Date(r.lastSeen * 1000).toLocaleString("pt-BR")}</span>
              ),
            },
            {
              label: "Expira",
              render: (r) => (
                <span className="num">{new Date(r.expiresAt * 1000).toLocaleString("pt-BR")}</span>
              ),
            },
            { label: "IP", render: (r) => <span className="num">{r.remoteIp || "N/D"}</span> },
            {
              label: "Cliente",
              render: (r) => <span className="text-[10px]">{r.userAgent || "N/D"}</span>,
            },
          ]}
        />
      </Panel>
