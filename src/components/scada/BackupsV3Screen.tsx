...
export function BackupsV3Screen() {
  ...
  const load = async () => {
    try {
      setRows(await rcApi.backups.list());
      setError("");
    } catch (err) {
  ...
  const remove = async (row: BackupApi) => {
    ...
    try {
      await rcApi.backups.remove(row.id);
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao excluir backup.");
    }
  };
  ...
  return (
    <ScreenBody>
      <Stats
        items={[
          { icon: HardDriveDownload, label: "Backups", value: rows.length },
          {
            ...
            label: "Com resultado OK",
            value: rows.filter((row) => row.result === "OK").length,
            tone: "text-online",
          },
        ]}
      />
      <p className="rounded-md border border-alert/30 bg-alert/5 px-3 py-2 text-[11px] text-muted-foreground">
        Restauração não é executada pela própria API em produção. O restore continua no runbook/CLI
        da VM, com serviços parados, validação do arquivo e rollback. Isso evita restaurar o banco
        por uma requisição HTTP enquanto o sistema está ativo.
      </p>
      {error && (
      ...
      <Panel
        title="Backups persistidos"
        actions={
          <span className="flex gap-1">
            {admin && (
              <ActionBtn disabled={busy} onClick={() => void create()}>
                {busy ? "Executando…" : "Fazer backup agora"}
              </ActionBtn>
            )}
        ...
        {!rows.length ? (
          ...
          <ScadaTable
            ...
            columns={[
              ...
              {
                label: "Resultado",
                render: (r) => <Pill tone={r.result === "OK" ? "ok" : "warn"}>{r.result}</Pill>,
              },
              {
                label: "Ações",
                render: (r) => (
