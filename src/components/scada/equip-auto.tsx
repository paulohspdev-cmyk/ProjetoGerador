      ...
      <Panel title="Comunicação industrial">
        <div className="space-y-3 text-[13px]">
          <p className="rounded-md border border-border p-3">
            Rapid SCADA é o mestre industrial. Reverse TCP usa bridge; Modbus TCP/RTU-over-TCP/serial são provisionados diretamente no Communicator.
          </p>
            ...
            {generators.map((g) => (
              <div key={g.id} className="rounded-md border border-border p-3">
                <div className="flex items-center justify-between gap-2">
                  <b>{g.tag}</b>
                  <Pill tone={g.telemetrySource === "rapid_scada" ? "ok" : "muted"}>{g.telemetrySource || "none"}</Pill>
                </div>
          ...
          {!generators.length && <p className="py-8 text-center text-muted-foreground">Nenhum gerador cadastrado.</p>}
        </div>
      </Panel>
    </ScreenBody>
  );
  ...
  return (
    <ScreenBody>
      ...
      <p className="rounded-md border border-alert/30 bg-alert/5 px-3 py-2 text-[11px] text-muted-foreground">
        Motor fechado: somente generator_online / generator_offline / generator_alert → notify / work_order. START, STOP, MCB, GCB e paralelismo são rejeitados pelo backend deste motor.
      </p>
      ...
      {admin && (
        <Panel title={editing ? "Editar regra não industrial" : "Nova regra não industrial"}>
          <form onSubmit={save} className="grid gap-2 lg:grid-cols-5">
            ...
            <select value={triggerType} onChange={(e) => setTriggerType(e.target.value)} className="h-9 rounded-md border border-input bg-background px-2 text-sm">
            ...
            <select value={generator} onChange={(e) => setGenerator(e.target.value)} className="h-9 rounded-md border border-input bg-background px-2 text-sm">
              {generators.map((g) => <option key={g.id} value={g.tag}>{g.tag}</option>)}
            </select>
            <div className="grid grid-cols-2 gap-1">
              <select value={actionType} onChange={(e) => { setActionType(e.target.value); setActionValue(e.target.value === "notify" ? "panel" : "Inspeção"); }} className="h-9 rounded-md border border-input bg-background px-2 text-sm">
                <option value="notify">Notificar</option>
                <option value="work_order">Criar OS</option>
              </select>
              <input value={actionValue} onChange={(e) => setActionValue(e.target.value)} placeholder={actionType === "notify" ? "panel/email/..." : "Inspeção"} className="h-9 rounded-md border border-input bg-background px-2 text-sm" />
            </div>
            <div className="flex gap-1">
              <button className="h-9 rounded-md bg-primary px-3 text-sm font-bold text-primary-foreground">{editing ? "Salvar" : "Criar"}</button>
              {editing && <button type="button" onClick={reset} className="h-9 rounded-md border border-border px-3 text-xs">Cancelar</button>}
            </div>
          </form>
        </Panel>
      )}
      <Panel title="Regras de automação">
        {!rules.length ? (
          <p className="py-8 text-center text-sm text-muted-foreground">Nenhuma regra cadastrada.</p>
        ) : (
          <ScadaTable
            rows={rules}
            columns={[
              { label: "Regra", render: (r) => <b>{r.name}</b> },
              { label: "Gatilho", render: (r) => <span className="num text-[11px]">{r.trigger}</span> },
...
export function ExerciseScreen() {
  ...
  return (
    <ScreenBody>
      ...
      <p className="rounded-md border border-alert/30 bg-alert/5 px-3 py-2 text-[11px] text-muted-foreground">
        Este módulo é agenda/planejamento. Não existe execução automática de START, transferência, MCB, GCB ou paralelismo por esta tela.
      </p>
      {error && <p className="text-[11px] text-offline">{error}</p>}
      {can("create") && (
        <Panel title="Planejar exercício">
          <form onSubmit={onCreate} className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
            <input required value={when} onChange={(e) => setWhen(e.target.value)} placeholder="Data/hora ou descrição" className="h-9 rounded-md border border-input bg-background px-2 text-sm" />
            <input value={site} onChange={(e) => setSite(e.target.value)} placeholder="Site" className="h-9 rounded-md border border-input bg-background px-2 text-sm" />
            <button className="h-9 rounded-md bg-primary px-4 text-sm font-bold text-primary-foreground">Planejar</button>
          </form>
        </Panel>
      )}
      <Panel title="Planejamentos">
        <ScadaTable
          ...
          columns={[
            ...
            { label: "Estado", render: (r) => <Pill tone={r.enabled === false ? "muted" : "ok"}>{r.enabled === false ? "Cancelado" : "Planejado"}</Pill> },
  ...
  return (
    <ScreenBody>
      ...
      <Panel title="Fila real de notificações" actions={<span className="flex gap-1"><ActionBtn onClick={() => void testPanel()}>Testar painel</ActionBtn>{admin && <ActionBtn onClick={() => void process()}>Processar fila</ActionBtn>}</span>}>
        {error && <p className="text-[11px] text-offline">{error}</p>}
        {!rows.length ? (
          <p className="py-8 text-center text-sm text-muted-foreground">Fila vazia. E-mail/WhatsApp só enviam quando credenciais reais forem configuradas.</p>
        ) : (
          <ScadaTable
            ...
            columns={[
              ...
              { label: "Estado", render: (r) => <Pill tone={r.status === "sent" ? "ok" : r.status === "failed" ? "err" : "warn"}>{r.status}</Pill> },
