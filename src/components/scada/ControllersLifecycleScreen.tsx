...
export function ControllersLifecycleScreen() {
  ...
  const controllerRows = useMemo<ControllerRow[]>(
    () =>
  ...
  const connectionRows = useMemo<ConnectionRow[]>(
    () =>
      topology.assets.flatMap((asset) =>
        (asset.controllers ?? []).flatMap((controller) =>
  ...
  const removeAsset = async (row: AssetV3) => {
    if (row.legacy_generator_id) {
      setError("Asset de gerador legado deve ser retirado pelo lifecycle do gerador.");
      return;
    }
    if (row.enabled) {
      setError("Desative o asset antes de excluí-lo.");
      return;
    }
