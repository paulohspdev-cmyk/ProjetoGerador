...
export function CompactCard({ gen }: { gen: Generator }) {
  ...
  const active = gen.status === "online";
  const batt = gen.battery ?? 0;
  const src = controllerImageSrc(gen.controller);
  const freq = configured ? `${fmt(gen.frequency ?? 0)} Hz` : "—";
  const hours = configured ? `${fmt(gen.runHours)} h` : "—";
  const maint = configured ? `${fmt(gen.maintenance, 0)} h` : "—";
  const lat = gen.latency != null ? `${gen.latency} ms` : "—";
  const latTone = !gen.latency ? undefined : gen.latency > 600 ? "text-offline" : gen.latency > 350 ? "text-alert" : "text-online";

  return (
    <article
      className={cn(
        "flex min-w-0 flex-col rounded-lg border bg-card p-2.5",
        active && "border-online/55 shadow-[0_0_12px_rgba(0,230,118,0.12)]",
      ...
      <div className="mt-2 flex items-start gap-2">
        ...
        {configured ? (
          <div className="min-w-0 flex-1 space-y-1">
            <Metric icon={<Network className="size-3" />} label="IP" value={gen.ip} />
            <Metric
              icon={<IconBattery size={12} />}
              label="Bateria"
              value={gen.battery ? `${fmt(batt)} V` : "—"}
              tone={!gen.battery ? undefined : batt < 12 ? "text-alert" : "text-online"}
            />
            <Metric
              icon={<IconBolt size={12} />}
              label="Frequência"
              value={gen.status === "offline" ? "—" : freq}
              tone={active ? "text-online" : "text-muted-foreground"}
            />
            <Metric icon={<IconRunHours size={12} />} label="Tempo operação" value={hours} />
            <Metric icon={<Clock className="size-3" />} label="Manutenção" value={maint} />
