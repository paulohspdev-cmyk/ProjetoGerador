...
export function KpiStrip() {
  ...
  const cards = [
    {
      icon: Server,
      label: "Total de Geradores",
      value: total,
      sub: total ? "100% do parque" : "Parque vazio",
    ...
    },
    {
      ...
      sub: latencyRows.length
        ? `${latencyRows.length} medição(ões) disponível(is)`
        : "Sem canal/medição de latência",
      tone: "text-foreground",
