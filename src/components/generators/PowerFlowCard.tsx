...
export function IoBtn({
  ...
  return (
    <button
      type="button"
      disabled={disabled}
      title={disabled ? "Comando ainda não homologado" : undefined}
  ...
  return (
    <svg viewBox="0 0 230 400" className="flow-diagram" preserveAspectRatio="xMidYMid meet" aria-label="Fluxo de energia">
      <g transform="translate(80 40)">
        ...
        <text x="35" y="5" fontSize="15" fontWeight="bold" fill="#eef4f8" textAnchor="start">{fmt(gridHz)} Hz</text>
      </g>
      ...
      <g transform="translate(178 200)">
        ...
        <text x="16" y="14" textAnchor="middle" className="flow-load-card-value">{fmt(loadKw, 0)} kW</text>
      </g>
      ...
      <g transform="translate(80 360)">
        ...
        <text x="-35" y="5" fontSize="15" fontWeight="bold" fill="#eef4f8" textAnchor="end">{fmt(genHz)} Hz</text>
      </g>
...
export function PowerFlowCard({ gen }: { gen: Generator }) {
  ...
  const running = gen.rpm > 300;
  const mcb = gen.mcb;
  const gcb = gen.gcb;
  const mainsOk = Math.max(gen.mains.l1, gen.mains.l2, gen.mains.l3, gen.mains.l12) > 50;
  const gridHz = mainsOk ? 60 : 0;
  const genHz = running ? (gen.frequency ?? 0) : 0;
  const loadKw = gen.load;
  const rpm = gen.rpm;
  const oil = gen.oilPressure;
  const temp = gen.coolantTemp;
  const fuel = gen.status === "nao_configurado" ? 0 : gen.fuelLevel;
  const batt = gen.status === "nao_configurado" ? 0 : (gen.battery ?? 0);
  const alt = gen.alternatorVoltage;
  const mains = gen.mains;
  const genV = gen.gen;
  ...
  return (
    <article className="comap-panel">
      <header className="comap-header">
        <span className={cn("comap-logo", gen.status === "online" ? "online" : "offline")}>G</span>
      ...
      <section className="comap-block comap-flow">
        <div className="mb-1 flex items-baseline gap-3">
          ...
          <span className="comap-mode">MODE: {gen.mode}</span>
        </div>

        <div className="comap-sld">
          <div className="comap-sld-stage">
            <button type="button" disabled title="Paralelismo ainda não homologado" className="comap-prll cursor-not-allowed opacity-60" style={{ position: "absolute", left: 0, top: "1%", zIndex: 20 }}>
              PRLL<br />—
            </button>
            ...
            <div className="absolute left-[2%] top-[26%] z-[2] flex flex-col items-center gap-1">
              ...
              <div className="flex flex-col gap-1">
                <IoBtn label="I" tone="close" active={mcb} ariaLabel="Estado fechado MCB" />
                <IoBtn label="O" tone="open" active={!mcb} ariaLabel="Estado aberto MCB" />
              </div>
            </div>
            ...
            <div className="absolute left-[2%] top-[52%] z-[2] flex flex-col items-center gap-1">
              ...
              <div className="flex flex-col items-center gap-1">
                <IoBtn label="I" tone="close" active={gcb} ariaLabel="Estado fechado GCB" />
                <IoBtn label="O" tone="open" active={!gcb} ariaLabel="Estado aberto GCB" />
              </div>
            </div>
            ...
            <div className="absolute bottom-[3%] right-[1%] z-10 flex flex-col gap-2">
              <button type="button" className="comap-start" disabled={!canOperate || commandBusy} onClick={() => void runCommand("start")} aria-label="Partir gerador">
                START
              </button>
              <button type="button" className="comap-stop" disabled={!canOperate || commandBusy} onClick={() => void runCommand("stop")} aria-label="Parar gerador">
                STOP
              </button>
            </div>
          </div>
      ...
      <section className="comap-block shrink-0 px-2 py-1.5">
        ...
        <EngineRow icon={<IconBolt />} label="Alternator Volt." value={`${fmt(alt)} V`} pct={(alt / 460) * 100} />
        <EngineRow icon={<IconClock />} label="Maintenance" value={`${fmt(gen.maintenance, 0)} h`} pct={(gen.maintenance / 250) * 100} kind="maint" />
      ...
      <section className="comap-block comap-rpm-block shrink-0 px-1 pt-1">
        ...
        <div className="comap-rpm-wrap"><RpmGauge value={rpm} /></div>
      </section>
