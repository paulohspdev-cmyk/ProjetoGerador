...
function battery(g: Generator) {
  return hasMetric(g, "battery_voltage") && g.battery != null ? `${fmt(g.battery)} V` : "N/D";
}
function frequency(g: Generator) {
  return hasMetric(g, "frequency") && g.frequency != null ? `${fmt(g.frequency, 2)} Hz` : "N/D";
}
function power(g: Generator) {
  return hasMetric(g, "power_kw") ? `${fmt(g.load)} kW` : "N/D";
}
function runHours(g: Generator) {
  return hasMetric(g, "run_hours") ? `${fmt(g.runHours)} h` : "N/D";
}
function maintenance(g: Generator) {
  return hasMetric(g, "maintenance_hours") ? `${fmt(g.maintenance, 0)} h` : "N/D";
}
function mode(g: Generator) {
  return hasMetric(g, "controller_mode_raw") ? g.mode : "N/D";
}
            ...
            {items.map((g) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={cn("num px-3 py-3 font-bold", modeClass(g.mode, modeKnown))}>
                    {mode(g)}
                  </td>
